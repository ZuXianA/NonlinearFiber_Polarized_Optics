...
a=a0;
for k=1:M
    a=a.*exp(1i*gamma*dL*(a.*conj(a)));
    A=fftshift(ifft(fftshift(a)));
    A=A.*exp(1i*0.5*beta2*dL*(2*pi*f).^2);    a=fftshift(fft(fftshift(A)));
end
...
% errT= sum((a.*conj(a)-a0.*conj(a0)).^2)/N;
% errF = sum((A.*conj(a)-A0.*conj(A0)).^2)/N;
% fprintf('The error between input and output at time domain is %g\n', errT);
% fprintf('The error between input and output at frequency domain is %g\n', errF);
